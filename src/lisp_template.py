def select_layers(layers: str):
    """
    Получает список слоев и при первом запуске
    скрывает все элементы на этих слоях.
    
    При последующем запуске выделяет и отображает
    все элементы первого слоя (их можно визуально
    проверить и сразу удалить).

    Таким образом осуществляется перебор всех слоем
    и визуальная проверка элементов на текущем слое. 
    """
    return f'''\
...
def set_layer(type_element):
    """
    Переключение текущего слоя на новый. Если нового
    слоя не существует, то происходит его создание
    """
    return f'''\
;; "{type_element}"
